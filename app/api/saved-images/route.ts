...
export async function GET() {
  try {
    console.log("[v0] Fetching saved images from database")
    ...
    // Fetch all SAVED images for the Gallery
    ...
    console.log("[v0] Found saved images:", savedImages?.length || 0)
    return NextResponse.json({ images: savedImages || [] })
  } catch (error) {
    console.error("[v0] Failed to fetch saved images:", error)
    return NextResponse.json({ images: [] })
  }
}
