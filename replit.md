...
3.  **Wavespeed AI**: Powers high-quality image generation and video creation. Video models include Express (5s or 8s, 480p) and Express HD (5s, 720p) using the wan-2.2 endpoint. Note: Wavespeed video API only supports 5 and 8 second durations.
4.  **HuggingFace**: Hosts various AI models, accessed with multiple API tokens for load distribution. The HuggingFace Router with fal-ai provider is used for the Basic video model (Wan-AI/Wan2.2-I2V-A14B-Diffusers, 3s or 5s videos, 1500 credits).
